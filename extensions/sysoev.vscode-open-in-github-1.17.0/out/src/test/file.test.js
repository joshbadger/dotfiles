...
suite("fileCommand # formatGitHubFileUrl", () => {
    ...
    test("should add ?plain=1 for markdown links", () => {
        const results = file.formatGitHubFileUrl("https://remote.url", "master", "rel/path/to/file.md", { start: 10 });
        assert.equal(results, "https://remote.url/blob/master/rel/path/to/file.md?plain=1#L10");
    });
});
